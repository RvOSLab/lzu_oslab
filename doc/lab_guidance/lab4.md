...
直到现在，我们的内核不受限制地跑在 CPU 上，可以直接访问任何一处物理地址。这时假如内核加载并运行用户程序，用户程序可以几乎无限制的读写物理内存。对于恶意程序，它可以杀死其它程序甚至可以杀死内核，将整个系统的资源据为己有;对于遵纪守法的程序，它必须小心翼翼地控制自己，避免无意中修改了其他程序。显然，这种无限制的环境既给了程序自由，又让程序处于随时可能被杀死的境地。

现代操作系统一般都实现了虚拟内存，程序跑在虚拟内存上，不直接访问物理内存，并且每个程序（进程）都有相同的地址空间。应用程序不需要再担心被别的程序杀死或者不小心写了别的程序的内存，也不再需要考虑自己在内存中的哪个位置。
...
考虑这样的情况，程序 A 访问虚拟地址 100，程序 B 也访问虚拟地址 100。对于程序 A，操作系统将虚拟地址 100 映射到物理地址 1000;对于程序 B，操作系统将物理地址映射到物理地址 2000。虽然两程序访问同一地址，但最终却访问到物理内存的不同位置。按照这种思路，让不同的程序拥有相同的虚拟地址空间，将其中的虚拟地址映射到不同的物理地址，这样就实现了”每个进程都有相同的地址空间“的抽象。相同虚拟地址可以映射到不同的物理地址，反过来，不同的虚拟地址也可以映射到同一物理地址，这样就实现了不同程序共享同一块内存。

经过以上讨论，相信同学们已经理解了实现虚拟内存的思路。虚拟内存需要软硬件协作才能够高效实现，并且硬件提供的机制制约着操作系统的实现方式，接下来介绍 RISCV 提供的虚拟内存机制。

# 实现虚拟内存的硬件基础

RISCV 提供了传统的基于页面的虚拟内存机制。当未开启分页时，程序发出访问内存的指令，处理器就访问该内存地址;开启分页时，处理器 *MMU*（*Memory Manegement Unit*）先查询页表，将接收到的地址（虚拟地址）转换为另一个地址（物理地址），然后再访问转换后的地址。硬件完成了查询页表，将虚拟地址转换为物理地址的过程，软件无法插手这个过程。硬件不能直接知道虚拟地址到物理地址的映射，软件主要的职责就是维护虚拟地址到物理地址的映射，并在恰当的时候将它写入到页表中。

现代处理器提供的分页机制基本都使用多级页表，将虚拟地址划分多个 *VPN*（*Virtual Page Numer*）和页内偏移（*offset*）。上一级页表中的页表项（*PTE*）指向下一级页表的起始物理地址，最后一级页表中页表项指向虚拟地址对应的物理地址。将页表看作页表项数组`pte page_table[]`，VPN 就是页表项数组的索引。以以下步骤查询页表获取物理地址：

1. 找到一级页表（页目录）的物理地址，这个地址保存在特殊的寄存器中
2. 分页分页根据 VPN 找到包含下级页表起始物理地址的页表项
...
可见，指令 TLB 的命中率接近 100%，数据 TLB 的命中率比较低，但也接近 50%。

每个进程都有自己独立的虚拟地址空间，为了避免进程切换后通过 TLB 访问到上一进程的映射，将虚拟地址转换为错误的物理地址，通常发生进程切换时要刷新 TLB（清除 TLB 缓存）。

了解了查询页表的过程再看本内核使用的分页方式。RV32 仅支持 Sv32（即上图展示的方式），RV64 目前仅支持 2 种分页方式，分别是支持 39 位虚拟地址空间的 Sv39 和支持 48 位虚拟地址空间的 Sv48，本内核使用最常用的 Sv39 方案。Sv39 支持 39 位虚拟地址空间，56 位物理地址空间（是的，物理地址空间往往大于虚拟地址空间，32 位机只支持 32 位地址空间指的是虚拟地址空间），每页 4096 字节，共$2^{27}$页，页目录物理地址存放在*控制寄存器* （*CSR*）stap 中。
...
*ASID*（*Address Space IDentifier*) 用来标识 TLB 词条中映射归属的地址空间，ASID 类似与进程 ID，每个进程都有一个 ASID。TLB 中多个进程的虚拟地址-物理地址映射共存，MMU 在查询 TLB 时会忽略不属于当前进程的映射，在进程切换时只需要修改 ASID，不需要刷新 TLB，降低了进程切换的成本。本实验不使用 ASID。
...
虚拟页大小和物理页相同，每页都是 4096 字节，因此页內偏移必须是$0 - 4095$，占据虚拟地址低 12 位，共 $2^{27}$ 页。为了确保每个页表大小相同，只能将剩下的 27 位三分，因此每个 VPN 9 位，一个页表共有 512 项，占据一页。

页表项包含下一级页表地址或虚拟地址对应的物理地址，但不会直接将物理地址写进去，这样占用空间太大了，通常要求页表物理地址 4K 对齐，省去页内偏移，用这段空间存放标志位。Rv39 页表项结构如下：
...
| :-----------: | :----------------------------------------------------------: |
|      RSW      |                  供操作系统使用，硬件会忽略                  |
|   D(Dirty)    |              自上次清除 D 未以来页面是否被写入               |
|   A(Acess)    |              自上次清除 A 位依赖页面是否被访问               |
|   G(Global)   |  是否对所有虚拟地址均有效，通常标记内核的页表，本实验不使用  |
|    U(User)    | 是否是用户所有的页。置位则只有 U-mode 可访问，否则只有 S-mode 客可访问 |
| X(eXecutable) | 是否可执行。U-mode 只能执行 U 位被置位的页中的代码，S-mode 只能执行清除 U 位的页中的代码 |
|  W(Writable)  | 是否可写。U-mode 只能写 U 位被置位的页，S-mode 只能写清除了 U 位的页 |
|  R(Readable)  | 是否可读。U-mode 只能写 U 位被置位的页，S-mode 只能写清除了 U 位的页 |
|  V(present)   |         存在位。若 V 位清空，表示该页不在物理内存中          |
...
RWX 三个标志有特殊用法，可以实现**大页模式**。当中间页表 RWX 位均为 0 时，CPU 将直接提取出 PTE 中的物理地址，不再进入下级页表查询。利用这个特性，将页目录中的 PTE RWX 位清空，一个 PTE 就可以映射 1 G 地址空间。
...
运行时，操作系统的加载器会分析可执行文件的格式，查找到程序入口点（C/C++ 程序一般是`main()`），并跳转执行执行。`main()`函数的地址就是通过解析 ELF 可执行文件获取到的。我们可以使用 readelf ELF 格式可执行文件的完整信息。
