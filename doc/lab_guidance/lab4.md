...
RISCV 提供了传统的基于页面的虚拟内存机制。当未开启分页时，程序发出访问内存的指令，处理器就访问该内存地址;开启分页时，处理器 *MMU*（*Memory Manegement Unit*）先查询页表，将接收到的（虚拟）地址转换为物理地址，然后再访问转换后的地址。硬件完成了查询页表，将虚拟地址转换为物理地址的过程，软件无法插手这个过程。硬件不能直接知道虚拟地址到物理地址的映射，软件主要的职责就是维护虚拟地址到物理地址的映射，并在恰当的时候将它写入到页表中。
...
|   A(Access)    |              自上次清除 A 位依赖页面是否被访问               |
|   G(Global)   |  是否对所有虚拟地址均有效，通常标记内核的页表，本实验不使用  |
|    U(User)    | 是否是用户所有的页。置位则只有 U-mode 可访问，否则只有 S-mode 可访问 |
