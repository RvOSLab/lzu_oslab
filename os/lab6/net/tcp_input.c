...
\**/
static int tcp_clean_retransmission_queue(struct sock *sk, uint32_t una) {
...
tcp_process(struct sock *sk, struct tcphdr *th, struct sk_buff *skb)
{
	...
	int tcp_data_len = iph->len - iph->ihl * 4 - th->hl * 4;
	...
	if (!tcp_packet_filter(tsk, th, skb)) {
		if (!th->rst) {
			tcb->rcv_nxt = th->seq + tcp_data_len;
