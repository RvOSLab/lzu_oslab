...
struct spinlock cl_lock;
struct spinlock es_lock;
...
inline void tcp_established_or_syn_recvd_socks_enqueue(struct sock *sk) {
	acquire_lock(&es_lock);
	list_add_tail(&sk->link, &tcp_establised_or_syn_recvd_socks);
	release_lock(&es_lock);
}


inline void tcp_established_or_syn_recvd_socks_remove(struct sock *sk) {
	acquire_lock(&es_lock);
	list_del_init(&sk->link);
	release_lock(&es_lock);
}

inline void tcp_connecting_or_listening_socks_enqueue(struct sock *sk) {
	acquire_lock(&cl_lock);
	list_add_tail(&sk->link, &tcp_connecting_or_listening_socks);
	release_lock(&cl_lock);
}

inline void tcp_connecting_or_listening_socks_remove(struct sock *sk) {
	acquire_lock(&cl_lock);
	list_del_init(&sk->link);
	release_lock(&cl_lock);
}
...
struct sock *tcp_alloc_sock() {
	struct tcp_sock *tsk = kmalloc(sizeof(struct tcp_sock));
...
static void tcp_clear_queues(struct tcp_sock *tsk) {
	acquire_lock(&tsk->ofo_queue.lock);
	skb_queue_free(&tsk->ofo_queue);
	release_lock(&tsk->ofo_queue.lock);
}
...
\**/
static int tcp_port_used(uint16_t pt) {
...
int tcp_read(struct sock *sk, void *buf, const uint32_t len) {
	...
	switch (sk->state) {
	case TCP_CLOSE:
		kprintf("error: connectin does not exists\n");
		goto out;
	case TCP_LISTEN:
	case TCP_SYN_SENT:
	...
	case TCP_LAST_ACK:
	case TCP_TIME_WAIT:
		kprintf("error:  connection closing\n");
		goto out;
	default:
		goto out;
	}
...
int tcp_close(struct sock *sk) {
	...
	case TCP_CLOSE_WAIT:
		...
		break;
	default:
		kprintf("Unknown TCP state for close\n");
		return -1;
	}
	return 0;
}
...
int tcp_v4_connect(struct sock *sk, const struct sockaddr_in *addr) {
	...
	uint32_t daddr = ((struct sockaddr_in *)addr)->sin_addr;
	int rc = 0;

	sk->dport = ntohs(dport);
	sk->sport = tcp_generate_port();			    /* 伪随机产生一个端口 */
	sk->daddr = ntohl(daddr);
	sk->saddr = netdev->addr;			            /* sk中存储的是主机字节序 */
...
static struct sock* tcp_accept(struct sock* sk) {
	...
	while (list_empty(&tsk->accept_queue)) {
		sleep_on(&tsk->wait);	/* 等待被唤醒 */
	}

	
	newtsk = tcp_accept_dequeue(tsk);
	newtsk->tcb.rcv_wnd = 444777;
...
\**/
static struct sock*
tcp_lookup_establised_or_syn_recvd_sock(uint32_t src, uint16_t sport,
...
\**/
static struct sock *tcp_lookup_connecting_or_listening_sock(uint32_t src, uint16_t sport) {
	struct sock *sk;
	struct list_head *item;
	acquire_lock(&cl_lock);
	list_for_each(item, &tcp_connecting_or_listening_socks) {
...
\**/
static int tcp_connection_exist(uint32_t dst, uint16_t dport) {
	struct sock *sk;
	struct list_head *item;
	list_for_each(item, &tcp_establised_or_syn_recvd_socks) {
		sk = list_entry(item, struct sock, link);
