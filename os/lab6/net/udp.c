...
\**/
int udp_data_dequeue(struct udp_sock *usk, void *user_buf, 
    const uint32_t userlen, struct sockaddr_in *saddr)  {
...
\**/
int udp_data_enqueue(struct udp_sock *usk, struct udphdr *udphd, struct sk_buff *skb) {
	struct sock *sk = &usk->sk;
	skb_queue_tail(&sk->receive_queue, skb);
	return 0;
}
