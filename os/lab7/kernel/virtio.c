...
void virtio_queue_init(volatile struct virtio_device *device, uint64_t is_legacy) {
    ...
    // for(page_ptr += PAGE_SIZE; page_ptr < VIRTQ_END_ADDRESS; page_ptr += PAGE_SIZE) {
    //     get_empty_page(page_ptr, KERN_RW);
    // }
    // memset((uint8_t *)VIRTQ_START_ADDRESS, 0, VIRTQ_LENGTH);
    // 5. set queue num
    ...
    virtio_queue.avail = (struct virtq_avail *) (virtio_queue.desc  + VIRTQ_RING_NUM);
    virtio_queue.used  = (struct virtq_used *) (page_ptr + 16 * VIRTQ_RING_NUM + 6 + 2 * VIRTQ_RING_NUM);
    if(is_legacy) {
        device->guest_page_size = PAGE_SIZE;
        device->queue_pfn = virtq_phy_addr / PAGE_SIZE;
    } else {
        device->queue_desc = virtq_phy_addr;
        device->queue_driver = virtq_phy_addr + 16 * VIRTQ_RING_NUM;
        device->queue_device = virtq_phy_addr + 16 * VIRTQ_RING_NUM + 6 + 2 * VIRTQ_RING_NUM;
        // 7. set queue ready
...
void virtio_test(volatile struct virtio_device *device) {
    ...
    virtio_queue.avail->ring[virtio_queue.avail->idx % VIRTQ_RING_NUM] = 0;
    __sync_synchronize();
