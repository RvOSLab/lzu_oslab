...
void virtio_net_init(volatile struct virtio_device *device, uint64_t is_legacy) {
    volatile struct virtio_net_config *net_config;
    // 1. reset device
    ...
    net_config = (volatile struct virtio_net_config *)((uint64_t)device + VIRTIO_NET_CONFIG_OFFSET);
    kprintf(
...
void virtio_net_test(volatile struct virtio_device *device) {
